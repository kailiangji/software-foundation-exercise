...
Fixpoint many_helper {T} (p : parser T) acc steps xs :=
  ...
  | 0, _ =>
    NoneE "Too many recersive calls"
  | _, NoneE _ =>
    SomeE ((rev acc), xs)
  | S steps', SomeE (t, xs') =>
...
Fixpoint parseAtomicExp (steps:nat) (xs : list token) :=
  match steps with
  | 0 => NoneE "Too many recursive calls"
  | S steps' =>
...
Definition testParsing {X : Type} (p : nat -> list token -> optionE (X * list token)) (s : string) :=
